...
export default function StudioPage() {
  ...
  const privacyScore = useMemo(() => {
    if (!engineOutput) return null;
    const mods = new Set(selectedModuleIds);
    let score = 100;
    if (scenario.unlinkabilityGoal !== "none" && !mods.has("reblind_rerandomize")) score -= 40;
    if (scenario.antiReplay !== "none" && !mods.has("verifier_challenge_nonce")) score -= 25;
    if (scenario.deviceBinding === "required" && !mods.has("device_binding")) score -= 25;
          ...
          <div className="flex flex-wrap items-center gap-4 p-4 bg-white border rounded-xl">
            ...
            {privacyScore !== null && (
              <div className="flex items-center gap-2">
                ...
                <Badge variant={privacyScore >= 80 ? "success" : privacyScore >= 50 ? "warning" : "destructive"}>
                  {privacyScore}/100
                </Badge>
              </div>
            )}
